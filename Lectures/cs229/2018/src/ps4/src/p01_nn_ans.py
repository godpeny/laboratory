...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    grad_wrt_weights = np.zeros(conv_W.shape)

    grad_wrt_data = np.zeros(data.shape)

    for x in range(input_width - conv_width + 1):
        for y in range(input_height - conv_height + 1):
            for output_channel in range(conv_channels):
                grad_wrt_weights[output_channel, :, :, :] += output_grad[output_channel, x, y] * data[:,
                                                                                                 x:(x + conv_width),
                                                                                                 y:(y + conv_height)]
                grad_wrt_data[:, x:(x + conv_width), y:(y + conv_height)] += output_grad[output_channel, x, y] * conv_W[
                                                                                                                 output_channel,
                                                                                                                 :, :,
                                                                                                                 :]

    grad_wrt_bias = np.sum(output_grad, axis=(1, 2))

    return (grad_wrt_weights, grad_wrt_bias, grad_wrt_data)
    ...
    for x in range(0, input_width, pool_width):
        for y in range(0, input_height, pool_height):
            for input_channel in range(input_channels):
                data_ = data[input_channel, x:x + pool_width, y:y + pool_height]
                i, j = np.unravel_index(np.argmax(data_), data_.shape)
                grad[input_channel, x + i, y + j] = output_grad[input_channel, x // pool_width, y // pool_height]
    return grad
...
def backward_linear(weights, bias, data, output_grad):
    ...
    """
    grad_wrt_weights = np.outer(data, output_grad)
    grad_wrt_bias = output_grad
    grad_wrt_data = output_grad @ weights.T
...
def backward_prop(data, labels, params):
    ...
    gradient["W2"], gradient["b2"], grad_flattened = backward_linear(W2, b2, flattened, grad_logits)

    grad_first_after_relu = np.reshape(grad_flattened, first_after_relu.shape)
    grad_first_max_pool = backward_relu(first_max_pool, grad_first_after_relu)
    grad_convolution = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, grad_first_max_pool)

    gradient["W1"], gradient["b1"], _ = backward_convolution(W1, b1, data, grad_convolution)
...
def run_train(all_data, all_labels, backward_prop_func):
    ...
    fig.savefig('output/train_ans.png')
